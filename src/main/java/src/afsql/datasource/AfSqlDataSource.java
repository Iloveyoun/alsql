...
import src.afsql.exception.AfSqlException;
...
/**
 * AfSqlDataSource提供连接池的一个简易实现
 */
public class AfSqlDataSource implements DataSource {
	...
	public AfSqlDataSource() {
	}

	public AfSqlDataSource(String driverClassName, String url, String username, String password, boolean ispool, int init,
						   int min, int max) {
		super();
	...
	public AfSqlDataSource(AfSqlConfig c) {
		super();
		this.driverClassName = c.getDriverClassName();
		this.url = c.getUrl();
		this.username = c.getUsername();
		this.password = c.getPassword();
		this.ispool = c.getIspool();
		this.init = c.getInit();
		this.min = c.getMin();
		this.max = c.getMin();
		this.loadDriver();
	...
	private void loadDriver() {
		try {
			...
			throw new AfSysException("数据库驱动加载异常：" + driverClassName, e);
		}
	}
	...
	private Connection loadConnection() {
		try {
			...
			return new AfSqlConnectionProxy(connection).setAfSqlDataSource(this).getConnection();// 返回代理对象
		} catch (SQLException e) {
			throw new AfSqlException("创建连接失败：", e);
		}
	}

	// 尝试释放一条链接
	private void closeConn(Connection conn) {
		((AfSqlConnectionProxy) Proxy.getInvocationHandler(conn)).closeClose();// 真正的关闭
	}
	...
	public void callBackConn(Connection conn) {
		try {
			...
			throw new AfSqlException("回收连接失败", e);
		}
	}
